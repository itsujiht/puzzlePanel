...
export default function Game() {
    ...
    if (win) {
        statusmes = 'Excellent! Click right grid to start next phase.';
    } else {
        statusmes = diff[1] - isFront.length + 1 + ' times left.';
        if (diff[1] - isFront.length + 1 === 0) {
            if (life) {
                statusmes += ' Click right grid to restart.'
            } else {
                statusmes += ' Game Over. Yout Score is ' + diff[1] + '.'; 
            }
